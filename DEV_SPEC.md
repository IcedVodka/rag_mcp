...
- **组件配置卡片**：读取 `Settings`，展示当前可插拔组件的配置状态：
    - LLM：provider + model（如 `anthropic / claude-3-5-sonnet-20241022`）
...
|---------|---------|------|---------|------|
| A1 | 初始化目录树与最小可运行入口 | [x] | 2026-03-18 | 目录结构、main.py、pyproject.toml、prompts |
| A2 | 引入 pytest 并建立测试目录约定 | [x] | 2026-03-18 | 8个冒烟测试通过 |
| A3 | 配置加载与校验（Settings） | [x] | 2026-03-18 | Settings类、validate_settings、17个测试 |
...
| D6 | Reranker（Core 层编排 + Fallback） | [x] | 2026-03-18 | 18个测试通过，支持fallback检测与trace记录 |
| D7 | 脚本入口 query.py（查询可用） | [x] | 2026-03-18 | CLI脚本实现，支持verbose/no-rerank等参数 |
...
- **目标**：实现 `AnthropicVisionLLM`，支持通过 Anthropic API 调用 Claude-3 系列进行图像理解。
- **修改文件**：
  - `src/libs/llm/anthropic_vision_llm.py`
  - `tests/unit/test_anthropic_vision_llm.py`（mock HTTP，不走真实 API）
- **实现类/函数**：
  - `AnthropicVisionLLM(BaseVisionLLM)`：实现 `chat_with_image` 方法
  - 支持 Anthropic 配置：`base_url`, `api_key`, `model`
- **验收标准**：
  - provider=anthropic 且配置 vision_llm 时，`LLMFactory.create_vision_llm()` 可创建 Anthropic Vision LLM 实例。
  - 支持图片路径和 base64 两种输入方式。
  - 图片过大时自动压缩至 `max_image_size` 配置的尺寸（默认2048px）。
  - API 调用失败时抛出清晰错误。
  - mock 测试覆盖：正常调用、图片压缩、超时、认证失败等场景。
- **测试方法**：`pytest -q tests/unit/test_anthropic_vision_llm.py`。
